...
class ChangePasswordCommand implements SimpleCommand {
    ...
    private void finalizePasswordChange(AuthenticationContext authCtx) {
        ...
        // v1.0.4+ Invalidate ALL sessions for this username (including current player) after password change
        ctx.authCache().removeAuthorizedPlayer(authCtx.player().getUniqueId());
        java.util.List<java.util.UUID> endedSessions =
                ctx.authCache().endAllSessionsForUsername(authCtx.username());

        // v1.0.4+ audit password change
        AuditLogger.logPasswordChange(authCtx.username(), authCtx.player().getUniqueId(),
                PlayerAddressUtils.getPlayerIp(authCtx.player()));

        // v1.0.4+ disconnect ALL players with this username (including current) so they re-login
        Component reloginMsg = Component.text(
                ctx.messages().get("auth.password_changed_relogin"),
                NamedTextColor.YELLOW);
        ctx.plugin().getServer().getAllPlayers().stream()
                .filter(p -> p.getUsername().equalsIgnoreCase(authCtx.username()))
                .forEach(p -> {
                    p.disconnect(reloginMsg);
        ...
        if (ctx.logger().isInfoEnabled()) {
            ctx.logger().info(AUTH_MARKER,
                    ...
                    authCtx.username(), endedSessions.size());
        }
    }
}
