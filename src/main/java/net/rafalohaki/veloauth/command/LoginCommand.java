...
class LoginCommand implements SimpleCommand {
    ...
    private void handleFailedLogin(AuthenticationContext authContext) {
        ...
        // v1.0.4+ audit failed login
        String reason = blocked ? "brute_force_blocked" : "invalid_password";
        AuditLogger.logLoginFailure(authContext.username(),
                PlayerAddressUtils.getPlayerIp(authContext.player()), reason);

        if (blocked) {
            authContext.player().sendMessage(ctx.sm().bruteForceBlocked());
            AuditLogger.logBruteForceBlock(authContext.username(),
                    PlayerAddressUtils.getPlayerIp(authContext.player()),
                    ctx.settings().getBruteForceMaxAttempts());
