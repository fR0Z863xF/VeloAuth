...
final class PostAuthFlow {
    ...
    static boolean execute(CommandContext ctx, AuthenticationContext authContext,
        ...
        // v1.0.4+ enforce concurrent-session limit; if false, abort
        boolean sessionStarted = ctx.authCache().startSession(p.getUniqueId(), authContext.username(),
                PlayerAddressUtils.getPlayerIp(p));
        if (!sessionStarted) {
            ctx.authCache().removeAuthorizedPlayer(p.getUniqueId());
            p.sendMessage(Component.text(
                    ctx.messages().get("auth.concurrent_session_limit"),
                    NamedTextColor.RED));
            return false;
        }
        ...
        // v1.0.4+ audit
        if ("login".equalsIgnoreCase(operationName) || "logged in".equalsIgnoreCase(operationName)) {
            AuditLogger.logLoginSuccess(authContext.username(), p.getUniqueId(),
                    PlayerAddressUtils.getPlayerIp(p));
        } else if ("registration".equalsIgnoreCase(operationName) || "registered".equalsIgnoreCase(operationName)) {
            AuditLogger.logRegistration(authContext.username(), p.getUniqueId(),
                    PlayerAddressUtils.getPlayerIp(p));
        }
