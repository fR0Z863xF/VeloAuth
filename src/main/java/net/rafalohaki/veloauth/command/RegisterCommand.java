...
import net.rafalohaki.veloauth.audit.AuditLogger;
...
class RegisterCommand implements SimpleCommand {
    ...
    private void processRegistration(Player player, String password) {
        ...
        try {
            ...
            // v1.0.4+ block premium players from registering offline accounts
            String playerIpForPremiumCheck = PlayerAddressUtils.getPlayerIp(authContext.player());
            if (ctx.authCache().isPlayerAuthorized(authContext.player().getUniqueId(), playerIpForPremiumCheck)) {
                CachedAuthUser cachedUser = ctx.authCache().getAuthorizedPlayer(authContext.player().getUniqueId());
                if (cachedUser != null && cachedUser.isPremium()) {
                    authContext.player().sendMessage(Component.text(
                            ctx.messages().get("auth.premium_cannot_register"),
                            NamedTextColor.RED));
                    if (ctx.logger().isWarnEnabled()) {
                        ctx.logger().warn("[PREMIUM_REGISTER_BLOCKED] Premium player {} attempted to register offline account from IP {}",
                                authContext.player().getUsername(), playerIpForPremiumCheck);
                    }
                    AuditLogger.logAdminAction(authContext.player().getUsername(),
                            "PREMIUM_REGISTER_ATTEMPT_BLOCKED", authContext.player().getUsername());
                    return;
                }
            }
